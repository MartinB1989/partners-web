    ...
    <div v-else>
      ...
      >
        ...
        <v-select
          ...
          :disabled="selectedCategories.length >= 3 && !categorySelector.level1"
      ...
      <div class="d-flex justify-end mt-4">
        <v-btn
          v-if="selectedCategories.length < 3 && canAddMore"
          color="primary"
          variant="outlined"
      ...
      <div v-if="selectedCategories.length > 0" class="mt-6">
        <h4 class="text-subtitle-2 mb-2">Categorías seleccionadas:</h4>
        <v-chip-group>
          <v-chip
...
const updateSelectedCategories = () => {
  ...
  emit('update:categories', categories.map(c => c.id))
}
...
onMounted(async () => {
  ...
  // Si hay categorías iniciales, configurar los selectores
  if (props.initialCategories && props.initialCategories.length > 0) {
    // Implementar lógica para inicializar categorías si es necesario
  }
})
</script> 